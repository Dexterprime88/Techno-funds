...
def analyze_option_chain(csv_path, spot_price=None):
    ...
    numeric_cols = [
        ...
        'Put Time Value', 'Put LTP', 'Strike', 'IV', 'Call LTP', 'Call Time Value',
        'Call Intrinsic Value(Spot)', 'Call Intrinsic Value(Fut)', 'Call Bid Price',
        'Call Offer Price', 'Call OI', 'Call Change %', 'Call OI Change',
        'Call Volume', 'Call POP %', 'Call Delta', 'Call Theta', 'Call Vega', 'Call Gamma'
    ]
    ...
    if spot_price is None:
        # Ensure there's data to avoid errors on empty dataframes
        if not df.empty and ('Call OI' in df.columns) and ('Put OI' in df.columns):
            spot_price = df.loc[(df['Call OI'] + df['Put OI']).idxmax()]['Strike']
        else:
            # Default or error handling if spot can't be determined
            spot_price = 0
    ...
    highest_oi_call_strike = df.loc[df['Call OI'].idxmax()]['Strike'] if not df.empty else 0
    highest_oi_put_strike = df.loc[df['Put OI'].idxmax()]['Strike'] if not df.empty else 0
    highest_volume_call_strike = df.loc[df['Call Volume'].idxmax()]['Strike'] if not df.empty else 0
    highest_volume_put_strike = df.loc[df['Put Volume'].idxmax()]['Strike'] if not df.empty else 0
    ...
    mgp_strike = df.loc[df['MGP Score'].idxmax()]['Strike'] if not df.empty else 0
    ...
    cwp_strike = df.loc[df['CWP Score'].idxmax()]['Strike'] if not df.empty else 0
    pwp_strike = df.loc[df['PWP Score'].idxmax()]['Strike'] if not df.empty else 0

    # --- Module 4: Dealer Theta-Box & Comfort Zone ---
    upper_wall = cwp_strike
    lower_wall = pwp_strike
    cwp_gamma = df.loc[df['Strike'] == cwp_strike, 'Call Gamma'] if not df.empty else pd.Series()
    pwp_gamma = df.loc[df['Strike'] == pwp_strike, 'Put Gamma'] if not df.empty else pd.Series()
    ...
    cwp_score = df.loc[df['Strike'] == cwp_strike, 'CWP Score'].iloc[0] if not df.empty and not cwp_gamma.empty else 0
    pwp_score = df.loc[df['Strike'] == pwp_strike, 'PWP Score'].iloc[0] if not df.empty and not pwp_gamma.empty else 0
    ...
    strikes = df['Strike']
    gamma_slope = np.polyfit(strikes, gamma_diff, 1)[0] if len(strikes) > 1 else 0
    ...
    # --- HTML Output ---
    output_filename = csv_path.replace('.csv', '_analysis.html')
    html = f"""
    <html>
        <head>
            <style>
        ...
        <body>
            <h1>NIFTY/BANKNIFTY End-of-Day Option Chain Analysis for {csv_path}</h1>
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Analyze NIFTY/BANKNIFTY End-of-Day Option Chain data.')
    parser.add_argument('csv_path', type=str, help='Path to the option chain CSV file.')
    parser.add_argument('--spot', type=float, help='Optional: Manually provide the current spot price.')
    args = parser.parse_args()

    analyze_option_chain(args.csv_path, spot_price=args.spot)
